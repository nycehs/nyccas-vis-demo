...
dir_raster <- "C:/Users/besp_/Desktop/leaflet/tifs/"
...
bc_leaflet <- leaflet() %>%
  ...
  addLegend(pal = palRaster_bc,
            ...
            title = htmltools::HTML(paste0("Annual Average<br/>",
                                           "Black Carbon (BC)<br/>",
                                           "<em>(in <th>&mu;</th>g/m<sup>3</sup>)</em>")),
            opacity = 1)
...
#no_raster2<- calc(no_raster2, sd)
no_raster2[no_raster2 < 10] <- 10
no_raster2[no_raster2 > 30] <- 30


no_vals2 <- data.frame(rasterToPoints(no_raster2))

palRaster_no_cuts <- colorNumeric(c("Yellow", "Red"), domain = c(9.9,30.1), na.color = "transparent", 
                              reverse = FALSE)
...
no2_raster2[no2_raster2 > 25.6] <- 25.6

no2_vals2 <- data.frame(rasterToPoints(no2_raster2))

palRaster_no2_cuts <- colorNumeric(c("Yellow", "Red"), domain = c(11.0,25.7), na.color = "transparent", 
                                  reverse = FALSE)
